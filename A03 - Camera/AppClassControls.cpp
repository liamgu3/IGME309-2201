...
void Application::CameraRotation(float a_fSpeed)
{
	...
	vector3 right = glm::cross(glm::normalize(m_pCamera->GetAbove() - m_pCamera->GetPosition()), glm::normalize(m_pCamera->GetTarget() - m_pCamera->GetPosition()));	//getting the right vector to rotate around
	quaternion q1 = glm::angleAxis(-fAngleY / 10, m_pCamera->GetAbove() - m_pCamera->GetPosition());	//quaternion for horizontal rotation and getting up vector to rotate around
	quaternion q2 = glm::angleAxis(-fAngleX / 10, right);												//quaternion for vertical rotation
	m_pCamera->SetTarget(((m_pCamera->GetTarget() - m_pCamera->GetPosition()) * q1 * q2) + m_pCamera->GetPosition());	//rotating the camera
